...
public class TmdbClient {
  ...
  public MovieSearchResponse pesquisarFilme(String query) {
    ...
    log.info(
        "✅ TMDB: Busca concluída query='{}' resultados={}", queryFinal, response.results().size());
    return response;
  }
  ...
  public MovieRecord buscarDetalhes(Long movieId) {
    ...
    MovieRecord response =
        restClient
            ...
            .uri(
                uriBuilder ->
                    uriBuilder.path("/movie/{id}").queryParam("language", "pt-BR").build(movieId))
            .retrieve()
  ...
  public List<CastRecord> buscarElenco(Long movieId) {
    ...
    CreditsResponse response =
        restClient.get().uri("/movie/{id}/credits", movieId).retrieve().body(CreditsResponse.class);
  ...
  public String buscarOndeAssistir(Long movieId) {
    ...
    if (response.results().containsKey("BR")) {
      ...
      if (brProviders != null && !brProviders.isEmpty()) {
        String providers =
            brProviders.stream().map(Provider::name).reduce((a, b) -> a + ", " + b).orElse("");
        log.info("✅ TMDB: Provedores encontrados movieId={} providers={}", movieId, providers);
        return providers;
      }
    }
